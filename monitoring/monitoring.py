...
and any numeric values will be converted to fields.
    ...
    measurement = SampleMeasurement('classifier', 55.22)
...
from securevars import get_securevar_by_key
...
class Client(object):
    ...
    def measure(self, obj):
        ...
        self._logger.info('Measuring %s', point)
...
class Influxdbd(Client):
    """
    This client will record the environment as part of every data point.
    """
    def __init__(self, app_name):
        logging.getLogger(__name__).info('Initializing Influxdbd ...')
        if os.environ.get('ALFRED_ENVIRONMENT') is None:
            password = get_securevar_by_key('influxdb_{}_write'.format(app_name))
        else:
